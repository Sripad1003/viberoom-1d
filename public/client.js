...
function setupEventListeners() {
  ...
  sidebarTabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      if (tab.classList.contains("active") && !document.body.classList.contains("sidebar-collapsed")) {
...
function setupSocketListeners() {
  ...
  socket.on("queue-update", ({ queue, currentVideoIndex: newIndex, currentTime, isPlaying }) => {
    ...
    // Handle first video being added to empty queue
    if (wasEmpty && currentVideo) {
      if (!player) {
        pendingVideoToLoad = currentVideo.videoId
        pendingSeekTime = currentTime || 0
        pendingAutoplay = isPlaying !== false // Default to true if not specified
    ...
    if (player && currentVideo) {
      ...
      pendingVideoToLoad = currentVideo.videoId
      pendingSeekTime = currentTime || 0
      pendingAutoplay = isPlaying !== false
    }
  })
  ...
  socket.on("sync-video", ({ currentVideo, currentTime, username, newUserId, isPlaying }) => {
    console.log("[sync-video] Received sync-video event:", {
      currentVideo,
      currentTime,
      username,
      newUserId,
...
function addVideoToQueue(videoId, title, thumbnail, channelTitle) {
  ...
  // If this was the first video and autoplay is enabled, start playing immediately
  if (wasEmpty) {
    hideVideoOverlay()
    ...
    if (autoPlayEnabled) {
      // Create player if it doesn't exist and start playing
      if (!player) {
        pendingVideoToLoad = videoId
        pendingSeekTime = 0
      ...
      // Create player but don't autoplay
      if (!player) {
        pendingVideoToLoad = videoId
        pendingSeekTime = 0
        pendingAutoplay = false
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest("#youtubeSearchInput") && !e.target.closest("#searchButton")) {
      searchResults.classList.remove("active")
    }
  })
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (e) => {
    if (!emojiPopup.classList.contains("hidden") && !emojiPopup.contains(e.target) && e.target !== emojiToggleBtn) {
