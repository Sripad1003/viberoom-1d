...
io.on("connection", (socket) => {
  ...
  socket.on("queue-update", ({ room, queue, currentVideoIndex }) => {
    ...
    const wasEmpty = rooms[room].queue.length === 0

    rooms[room].queue = queue
    rooms[room].currentVideoIndex = currentVideoIndex

    // If this is the first video being added to an empty queue, enable autoplay
    if (wasEmpty && queue.length > 0 && currentVideoIndex >= 0) {
      rooms[room].isPlaying = true
      rooms[room].currentTime = 0
    }

    console.log(
      `[Server] Queue updated in room ${room}: ${queue.length} videos, current index: ${currentVideoIndex}, isPlaying: ${rooms[room].isPlaying}`,
    )
